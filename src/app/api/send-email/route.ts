import { ethicalServiceTypes } from "@/constants/ethics";
...
export async function POST(request: NextRequest) {
  ...
  if (
    !ethicalDeclaration ||
    !ethicalServiceTypes.includes(
      serviceType as (typeof ethicalServiceTypes)[number],
    )
  ) {
    return NextResponse.json(
      { error: "Başvuru etik danışmanlık kapsamı dışında." },
      { status: 400 },
    );
  }
  ...
  const mailOptions: CreateEmailOptions = {
    ...
    text: `Adı: ${name}\nE-posta adresi: ${email}\nTelefon numarası: ${phone}\nDestek konusu: ${serviceType}\nEtik kapsam beyanı: Onaylandı\nMesaj: ${message}`,
  };
